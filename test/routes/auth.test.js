...
test('Deve criar usuário via signup', () => {
  return request(app).post('/auth/signup')
    .send({ name: 'Daniel', mail: `${Date.now()}@mail.com`, passwd: '123456' })
    .then((res) => {
...
test('Deve receber token ao logar', () => {
  ...
  return app.services.user.save({ name: 'Daniel Gallo', mail, passwd: '123456' })
...
test('Não deve autenticar usuário com senha errada', () => {
  ...
  return app.services.user.save({ name: 'Daniel Gallo', mail, passwd: '123456' })
    ...
    .then((res) => {
      ...
      expect(res.body.error).toBe('Usuário ou senha errada');
    });
});
